...
How a release of `framesig` is made, what evidence it leaves behind, how anyone
...
gh release download vX.Y.Z --repo ferinazumaDEV/framesig
...
gh attestation verify ./*.whl --repo ferinazumaDEV/framesig
gh attestation verify ./*.tar.gz --repo ferinazumaDEV/framesig

# 3. PyPI serves those same bytes, and says where they came from.
curl -s https://pypi.org/pypi/framesig/X.Y.Z/json \
...
curl -s -H 'Accept: application/vnd.pypi.integrity.v1+json' \
  https://pypi.org/integrity/framesig/X.Y.Z/FILENAME/provenance \
...
```bash
gh run download <run-id> --repo ferinazumaDEV/framesig --name dist --dir dist
gh release create vX.Y.Z --repo ferinazumaDEV/framesig --title vX.Y.Z \
